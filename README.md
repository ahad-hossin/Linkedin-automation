...
pedestrian-safety** news and research, drafts professional **LinkedIn** posts
about them, and stores the drafts in this repo. A static website (GitHub Pages)
lets you review every draft, edit anything, polish/translate it into LinkedIn
style, and publish to LinkedIn with one click. **Nothing is ever posted
automatically.**
...
.venv\Scripts\python -m src.main publish      # send any "queued" drafts to LinkedIn (dry-run without creds)
```
...
src/article.py    article body extraction (for news links)
src/brain.py      AI lanes + select / compose / polish prompts
src/store.py      writes docs/data/posts/<id>.json + index.json
src/publish.py    LinkedIn UGC Posts API
...
.github/workflows/publish.yml    on-demand LinkedIn posting
```
